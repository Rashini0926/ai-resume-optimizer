...
const CoverLetterSchema = new Schema<CoverLetterDocument>({ userId: { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true }, resumeId: { type: Schema.Types.ObjectId, ref: 'ResumeAnalysis' }, jobDescription: { type: String, required: true, trim: true, maxlength: 20000 }, tone: { type: String, required: true, enum: ['Professional', 'Enthusiastic', 'Formal', 'Friendly'] }, content: { type: String, required: true, trim: true, maxlength: 20000 } }, { timestamps: true });
